...
exports.scrapeCommodityOnline = scrapeCommodityOnline;
...
async function getBrowser() {
    if (!_browser || !_browser.isConnected()) {
        _browser = await puppeteer_1.default.launch({
            headless: true,
            args: [
                '--no-sandbox',
...
async function scrapeCommodityOnline(commodity, state) {
    let page = null;
    try {
            ...
            highlightRows.forEach(div => {
                ...
                if (h4 && p) {
                    ...
                    if (label.includes('average'))
                        result.summary.avg = price;
                    else if (label.includes('lowest'))
                        result.summary.min = price;
                    else if (label.includes('costliest'))
                        result.summary.max = price;
                }
            });
            // 2. Table rows
            ...
            if (!result.summary.avg) {
                document.querySelectorAll('tr').forEach(tr => {
                    ...
                    if (numMatch) {
                        ...
                        if (text.toLowerCase().includes('avg'))
                            result.summary.avg = price;
        ...
        if (scraped.summary.avg) {
            ...
            const prices = scraped.markets.map((m) => m.price).filter((p) => p > 0);

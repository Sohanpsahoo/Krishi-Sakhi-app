...
exports.searchSchemes = exports.getScheme = exports.listSchemes = void 0;
...
async function fetchSchemesFromGemini(_state) {
...
async function seedSchemes(state) {
    ...
    const existingNational = await Scheme_1.default.countDocuments({ category: 'national' });
    ...
    if (state && state !== 'All India') {
        const existingState = await Scheme_1.default.countDocuments({ state, category: 'state' });
        if (existingState === 0) {
    ...
    if (toInsert.length > 0) {
        await Scheme_1.default.insertMany(toInsert);
    }
    // Fetch all
    const query = {};
    if (state && state !== 'All India')
        query.$or = [{ state }, { state: 'All India', category: 'national' }];
    return Scheme_1.default.find(query).sort({ category: 1, launch_year: -1 });
}
...
const listSchemes = async (req, res) => {
    try {
        ...
        const dbQuery = {};
        if (state && state !== 'All India')
            dbQuery.$or = [{ state }, { state: 'All India', category: 'national' }];
        if (category === 'national')
            dbQuery.category = 'national';
        if (category === 'state')
            dbQuery.category = 'state';
        let schemes = await Scheme_1.default.find(dbQuery).sort({ category: 1, launch_year: -1 });
        if (schemes.length === 0) {

...
exports.getRecommendationHistory = exports.getCropRecommendation = void 0;
...
const MODEL_CHAIN = geminiClient_1.GEMINI_MODELS; // ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-1.5-pro']
// ---------------------------------------------------------------------------
...
async function fetchWeather(lat, lon) {
    ...
    const cached = await GeoCache_1.default.findOne({ key: cacheKey });
    ...
    try {
        ...
        await GeoCache_1.default.findOneAndUpdate({ key: cacheKey }, { key: cacheKey, data: weather, updatedAt: new Date() }, { upsert: true });
...
async function callMLModel(payload) {
    // If the Python ML server is down, don't crash the entire request.
    // Instead, use a 2-second timeout and provide a realistic fallback.
    try {
        const ML_URL = ML_API_URL.replace('localhost', '127.0.0.1'); // Fix ipv6 node lag
        const { data } = await axios_1.default.post(`${ML_URL}/predict`, payload, {
            timeout: 2000,
        });
        ...
        console.warn("⚠️ ML API unavailable or timed out, using fallback crop model:", err.message);
        return {
            ...
            top3: [
                ...
                { crop: "maize", confidence: 0.50 }
            ],
            model_context: {
                soil: { nitrogen_kg_ha: payload.N, phosphorous_kg_ha: payload.P, potassium_kg_ha: payload.K, ph: payload.ph, type: payload.soil_type },
                weather: { temperature_c: payload.temperature, humidity_pct: payload.humidity, rainfall_mm: payload.rainfall },
                location: { city: payload.city, state: payload.state, country: payload.country, lat: payload.lat, lon: payload.lon },
                ml_recommendations: [
                    { rank: 1, crop: "wheat", confidence_pct: "85%" },
                    { rank: 2, crop: "rice", confidence_pct: "60%" },
                    { rank: 3, crop: "maize", confidence_pct: "50%" }
                ],
                primary_crop: "wheat"
            }
        };
...
function getSowingAdvice(crop, tempC, rainfall, humidity, stateName) {
...
function getYieldEstimate(crop, tempC, rainfall, humidity, soilN, soilP, soilK) {
    ...
    if (goodTemp && goodRain && goodNutrients)
        quality = "above-average";
    else if (!goodTemp || (!goodRain && rainfall < 20))
...
// ---------------------------------------------------------------------------
async function getGeminiExplanation({ modelContext, farmerName, userState, userDistrict, irrigationType, landSizeAcres }) {
  ...
  "detailedRecommendations": [
    ...
    for (const modelName of MODEL_CHAIN) {
        try {
            ...
            const geminiModel = (0, geminiClient_1.getGenAI)().getGenerativeModel({ model: modelName });
            ...
            const shouldTryNext = msg.includes("quota") ||
                msg.includes("429") ||
            ...
            lastError = err;
            if (shouldTryNext) {
                if ((0, geminiClient_1.isQuotaError)(err))
                    (0, geminiClient_1.rotateKey)();
            }
            else {
                throw err;
            }
...
const getCropRecommendation = async (req, res, next) => {
    try {
        ...
        const saved = await Recommendation_1.default.create({
            farmer: farmerId,
            farm: farmId || null,
...
exports.getCropRecommendation = getCropRecommendation;
// ---------------------------------------------------------------------------
...
const getRecommendationHistory = async (req, res, next) => {
    try {
        ...
        const recs = await Recommendation_1.default.find({ farmer: farmerId })
            .sort({ createdAt: -1 })
            .limit(20)
            .lean();

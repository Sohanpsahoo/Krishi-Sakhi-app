...
exports.getActivityInsights = exports.deleteActivity = exports.getActivity = exports.quickAdd = exports.listActivities = void 0;
...
const listActivities = async (req, res, next) => {
    try {
        ...
        if (req.query.farm_id)
            filter.farm = req.query.farm_id;
        if (req.query.type)
            filter.activity_type = req.query.type;
        if (req.query.date_from || req.query.date_to) {
            filter.date = {};
            if (req.query.date_from)
                filter.date.$gte = new Date(req.query.date_from);
            if (req.query.date_to)
                filter.date.$lte = new Date(req.query.date_to + 'T23:59:59');
        }
...
const getActivityInsights = async (req, res) => {
    try {
        ...
        if (!farmerId)
            return res.status(400).json({ message: 'farmer_id required' });
        const activities = await Activity_1.default.find({ farmer: farmerId })
            .populate('farm', 'name')
            .sort({ date: -1 })
            .limit(50);
...
exports.getActivityInsights = getActivityInsights;

...
exports.getSuggestions = exports.geminiChat = void 0;
...
async function callGeminiWithFallback(systemText, history, message) {
    ...
    for (const modelName of MODEL_CHAIN) {
        try {
            ...
            const geminiModel = (0, geminiClient_1.getGenAI)().getGenerativeModel({
                model: modelName,
                systemInstruction: {
                    role: "user",
...
// ---------------------------------------------------------------------------
const geminiChat = async (req, res, next) => {
    try {
        ...
        if (farmer_id) {
            try {
                const farmer = await Farmer_1.default.findById(farmer_id).lean();
                if (farmer) {
                    farmerInfo = `Farmer: ${farmer.name}, Location: ${farmer.village || ""}, ${farmer.district || ""}, ${farmer.state || "India"}`;
                    // Fetch Farms
                    const farms = await Farm_1.default.find({ farmer: farmer_id }).lean();
                    if (farms && farms.length > 0) {
                        farmContext = "FARMER'S FARMS:\n" + farms.map(f => `- Farm ID: ${f._id} | Name: ${f.name} (${f.land_size_acres} acres, ${f.irrigation_type || 'rain_fed'} ${f.soil_type ? ', ' + f.soil_type + ' soil' : ''})` +
                            (f.primary_crops ? ` Primary Crops: ${f.primary_crops}.` : '') +
                            (f.nitrogen_value ? ` NPK: ${f.nitrogen_value}/${f.phosphorus_value}/${f.potassium_value}, pH: ${f.soil_ph}` : '')).join("\n");
                    }
                    // Fetch Activities
                    const activities = await Activity_1.default.find({ farmer: farmer_id }).sort({ date: -1 }).limit(5).lean();
                    if (activities && activities.length > 0) {
                        activityContext = "RECENT FARM ACTIVITIES:\n" + activities.map((a) => `- ${new Date(a.date).toLocaleDateString()}: ${a.activity_type.toUpperCase()} - ${a.text_note}` + (a.amount ? ` (${a.amount})` : '')).join("\n");
                    }
                    ...
                    if (schemes && schemes.length > 0) {
                        schemeContext = "AVAILABLE GOVERNMENT SCHEMES:\n" + schemes.map((s) => `- ${s.name}: ${s.description.slice(0, 100)}...`).join("\n");
                    }
                    // Fetch Active Reminders
                    const reminders = await Reminder_1.default.find({ farmer: farmer_id, is_completed: false }).sort({ due_date: 1 }).limit(10).lean();
                    if (reminders && reminders.length > 0) {
                        reminderContext = "ACTIVE REMINDERS:\n" + reminders.map(r => `- ID: ${r._id} | Title: ${r.title} | Due: ${new Date(r.due_date).toLocaleDateString()} | Priority: ${r.priority} | Category: ${r.category}`).join("\n");
                    }
                }
                const latestRec = await Recommendation_1.default.findOne({ farmer: farmer_id })
                    .sort({ createdAt: -1 })
                    .lean();
...
- Warnings         : ${warnings}`.trim();
                ...
                if (crop && farmer?.state) {
                    const prices = await MarketPrice_1.default.find({
                        state: farmer.state,
                        commodity: { $regex: new RegExp(crop, 'i') }
                    }).sort({ arrival_date: -1 }).limit(3).lean();
                    if (prices && prices.length > 0) {
                        marketContext = "RECENT MARKET PRICES FOR " + crop.toUpperCase() + ":\n" + prices.map(p => `- Mandi: ${p.market} (${p.district}). Price: ₹${p.modal_price}/quintal (Min: ₹${p.min_price}, Max: ₹${p.max_price}) on ${new Date(p.arrival_date).toLocaleDateString()}`).join("\n");
                    }
                }
            }
            catch (e) {
...
<log_activity>
{
  ...
  "date": "YYYY-MM-DD"
   ...
   After logging, confirm to the user what was logged.`;
        ...
        const finalHistory = cleanHistory.filter((m) => m.parts[0].text !== message);
        console.log(`📜 History length: ${finalHistory.length}`);
        // ── Call Gemini with fallback chain ──
        const { text: fullText, modelUsed } = await callGeminiWithFallback(systemText, finalHistory, message);
        // ── Parse <cards> ──
        let reply = fullText;
        let cards = null;
        ...
        if (completeMatch && farmer_id) {
            try {
                ...
                if (compData.reminder_id) {
                    await Reminder_1.default.findByIdAndUpdate(compData.reminder_id, { is_completed: true });
...
const getSuggestions = async (req, res, next) => {
    try {
        ...
        if (farmer_id) {
            try {
                const rec = await Recommendation_1.default.findOne({ farmer: farmer_id })
                    .sort({ createdAt: -1 })
                    .lean();

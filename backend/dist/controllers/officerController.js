...
exports.getGovernmentHelplines = exports.getAIExperts = exports.saveAIExpert = exports.cancelConsultation = exports.listConsultations = exports.bookConsultation = exports.getOfficer = exports.listOfficers = void 0;
const Officer_1 = __importDefault(require("../models/Officer"));
...
async function seedOfficersForLocation(state, district) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const officer = await Officer_1.default.create({
            name,
            designation,
            department,
            specialization,
...
const listOfficers = async (req, res) => {
    try {
        ...
        let officers = await Officer_1.default.find(query).sort({ rating: -1 }).limit(30);
...
const getOfficer = async (req, res) => {
    try {
        const officer = await Officer_1.default.findById(req.params.id);
        if (!officer)
...
const bookConsultation = async (req, res) => {
    try {
        ...
        const populated = await Consultation_1.default.findById(consultation._id)
...
const cancelConsultation = async (req, res) => {
    try {
        const consultation = await Consultation_1.default.findByIdAndUpdate(req.params.id, { status: 'cancelled' }, { new: true });
        if (!consultation)
...
const saveAIExpert = async (req, res) => {
    try {
        const { name, designation, department, specialization, state, district, office_address, phone, email, available_hours, experience_years, languages, rating, is_available, consultation_fee } = req.body;
        if (!name || !state) {
            return res.status(400).json({ success: false, message: 'Name and state are required' });
        }
        // Check if already saved
        let officer = await Officer_1.default.findOne({ name, state });
        if (officer) {
            return res.json({ success: true, data: officer, existing: true });
        }
        officer = await Officer_1.default.create({
...
exports.saveAIExpert = saveAIExpert;
function getHardcodedExperts(state) {
    ...
    const getName = (i) => {
        if (!nameBank)
            return `Dr. Officer ${i + 1}`;
        if (Array.isArray(nameBank))
            return nameBank[i % nameBank.length];
...
// ─── 6) Experts list (hardcoded — no Gemini API) ────────────────────
const getAIExperts = async (req, res) => {
    try {
        const { state } = req.query;
        if (!state) {
            return res.status(400).json({ success: false, message: 'State is required' });
        }
        console.log(`📋 Returning default officers for ${state} (no Gemini)`);
        const experts = getHardcodedExperts(state);
        res.json({ success: true, data: experts, source: 'default' });
    }
    catch (error) {
...
exports.getAIExperts = getAIExperts;
...
const getGovernmentHelplines = async (req, res) => {
    try {
        const { state } = req.query;
...
exports.getGovernmentHelplines = getGovernmentHelplines;

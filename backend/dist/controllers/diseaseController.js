...
const detectDisease = async (req, res) => {
    try {
        ...
        let isHealthy = true;
        let detectionSource = null; // "cnn_model", "kindwise", or "gemini"
        // =====================================================================
        // STEP 1: Try Local CNN Model (Primary)
        // =====================================================================
        try {
            console.log('🤖 STEP 1: Attempting disease detection with local CNN model...');
            const cnnResponse = await axios_1.default.post(`${CNN_API_URL}/predict`, {
        ...
        if (!detectionSource && KINDWISE_API_KEY) {
            try {
                ...
                const kindwiseResponse = await axios_1.default.post('https://crop.kindwise.com/api/v1/identification?details=common_names,description,symptoms,treatment,prevention,cause', kindwisePayload, {
                    headers: {
        ...
        let remedies = null;
        let nextSteps = null;
        if (!(0, geminiClient_1.getCurrentKey)()) {
            console.error('⚠️ No Gemini API keys configured');
        }
        else {
            const modelsToTry = geminiClient_1.GEMINI_MODELS;
            ...
            if (!detectionSource) {
                try {
                    ...
                    for (const modelName of modelsToTry) {
                        try {
                            const model = (0, geminiClient_1.getGenAI)().getGenerativeModel({ model: modelName });
                            const imagePart = {
                                inlineData: {
                                    data: base64Image,
            ...
            `;
                    ...
                    for (const modelName of modelsToTry) {
                        try {
                            ...
                            if ((0, geminiClient_1.isQuotaError)(err))
                                (0, geminiClient_1.rotateKey)();
                            fs_1.default.appendFileSync('gemini_error.txt', `\n[${new Date().toISOString()}] ${modelName} Error: ${err.message}\n`);
                        }
                    }
                }
                catch (geminiRemedyError) {

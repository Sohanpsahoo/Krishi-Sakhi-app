...
// ---------------------------------------------------------------------------
export const sttTranslateBase64 = async (req: Request, res: Response) => {
  try {
    const apiKey = getSarvamApiKey();
    if (!apiKey) {
      return res.status(500).json({ success: false, message: 'SARVAM_API_KEY not configured' });
    ...
    const extMap: Record<string, string> = { 'audio/mp4': '.m4a', 'audio/wav': '.wav', 'audio/webm': '.webm', 'audio/mpeg': '.mp3', 'audio/aac': '.aac' };

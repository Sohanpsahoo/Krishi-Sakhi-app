...
export default function Officers() {
  ...
  // Fetch AI experts when state changes
  useEffect(() => {
    if (selectedState) {
      fetchAIExperts();
      fetchHelplines();
  ...
  const fetchAIExperts = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/officers/ai-experts?${params}`);
      const data = await res.json();
      if (data.success) {
        setOfficers(data.data || []);
        setExpertSource(data.source || 'ai');
      } else {
        setOfficers([]);
      }
    } catch (err) {
      console.error('Failed to fetch AI experts:', err);
      setOfficers([]);
    } finally {
      setLoading(false);
    }
  ...
  const submitConsultation = async (e) => {
    ...
    setBooking(true);
    try {
      // If this is an AI-generated expert (non-ObjectId _id), save to DB first
      ...
      if (typeof officerId === 'string' && (officerId.startsWith('ai_') || officerId.startsWith('fb_'))) {
        ...
        if (saveData.success && saveData.data?._id) {
          ...
          alert('Failed to register expert. Please try again.');
          setBooking(false);
          return;
        }
      }
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-gray-100 mb-6" style={{ animation: 'slideUp 0.3s ease-out' }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="flex items-end">
              <button onClick={fetchAIExperts} disabled={!selectedState || loading}
